...
export class AdminDashboardComponent {
  ...
  guardarCarta(): void {
    ...
    // Eliminamos el "as any" ya que 'carta' ahora cumple perfectamente con el tipo 'Product'
    this.productService.createProduct(this.carta).subscribe({
      next: (res: Product) => { //  Tipamos la respuesta limpia (Soluciona TS7006 de 'res')
        this.successAlert = `✅ ¡${res.name} añadida exitosamente al stock con ID ${res.number}!`;
        setTimeout(() => this.router.navigate(['/catalog']), 2000);
      },
      error: (err: HttpErrorResponse) => { // Tipamos el error de manera estricta (Soluciona TS7006 de 'err')
